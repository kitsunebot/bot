...
var computeResponse = (msg, key, replacer, skipPrefix)=> {
    var isPrivate = msg.channel.guild === undefined;
    return (isPrivate || skipPrefix ? '' : utils.replace(langfiles[config.languages.default]['username'], {username: (isPrivate ? msg.author.user.username : msg.member.nick)})) + ' ' + computeLangString(isPrivate ? msg.author.id : msg.channel.guild.id, key, isPrivate, replacer);
};
