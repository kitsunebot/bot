            ...
            request.get(channel.api_url, function (err, resp, body) {
                if (!err && [200, 304].indexOf(resp.statusCode) !== -1) {
                    ...
                    if (channel.status && body.stream === null) {
                        channel.update({status: false}).then(function () {
                            channel.getTwitchWatchers().then(function (watchers) {
                                watchers.forEach(function (watcher) {
                                    var msg = discordBot.channels.get('id', watcher.server_channel).messages.get('id', watcher.msg_id);
                                    if (msg !== null && msg !== undefined) msg.delete();
                                });
                            });
                        });
                            ...
                            channel.getTwitchWatchers().then(function (watchers) {
                                watchers.forEach(function (watcher) {
                                    utils.messages.sendMessage(discordBot.channels.get('id', watcher.server_channel), {
                                        ...
                                        if (!err) {
                                            discordBot.sendFile(watcher.server_channel, body.preview.medium);
                                            watcher.update({msg_id: msg.id});
                                        }
                                    });
                                });
                            });
