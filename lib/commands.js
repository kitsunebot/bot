            ...
            if (split.length === 3) {
                if (split[1] === 'add' || split[1] === 'crete') {
                            ...
                            if ([304, 200].indexOf(resp.statusCode) !== -1) {
                                db.models.TwitchChannel.findOrCreate({
                                    ...
                                    channel.getTwitchWatchers({where: {server_channel: msg.channel.id}}).then(function (watchers) {
                                        ...
                                        } else utils.messages.sendReply(msg, 'twitchWatcher.already_watched');
                                    });
                                });
                            } else utils.messages.sendReply(msg, 'twitchWatcher.already_watched');
                        } else utils.messages.sendReply(msg, 'unknown_error');
                    });
                    ...
                    db.models.TwitchChannel.find({where: {channel: split[2]}}).then(function (channel) {
                        ...
                        function notWatched() {
                            utils.messages.sendReply(msg, 'twitchWatcher.no_watcher');
                        }
                    });
                } else wrongArg();
            } else if (split.length === 2) {
