    ...
    handler: (gitlab)=> {
        if (eris.channelGuildMap[gitlab.channel] !== undefined) {
            if (gitlab.event === 'Push Hook') {
                eris.createMessage(gitlab.channel, lang.computeLangString(eris.channelGuildMap[gitlab.channel], 'gitlab.push', false, {
                    ...
                    commits: gitlab.payload.commits.map(commit=>lang.computeLangString(eris.channelGuildMap[gitlab.channel], 'gitlab.commit', false, {
