...
class Links_Blacklist extends Filter {
    constructor(blacklist,guild) {
        super(blacklist,guild);
        this.id = 'Links_Blacklist';
        this.enabled = false;
        ...
        return new Promise((resolve, reject)=> {
            ...
            if (matches.length === 0)resolve();
                ...
                for (let element in matches) {
                    ...
                    if (that.blacklist.includes(url.host)) {
                        reject({name: that.id});
                        return;
                    }
                }
...
module.exports = Links_Blacklist;